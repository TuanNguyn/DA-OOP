...
public class MenuKho {
    ...
    public static void themKho() {
        ...
        if (sach == null) {
            System.out.println("Loi: Ma sach khong ton tai.");
            return;
        }
        ...
        Kho khoMoi = new Kho(maKho, sach, soLuong, mucTonToiThieu, viTri);
        dsKho.them(khoMoi);
    ...
    public static void suaKho() {
        ...
        System.out.print("Ma sach moi (" + khoCanSua.getSach().getMaSach() + "): ");
        String maSachMoi = sc.nextLine();
        Sach sachMoi = khoCanSua.getSach();
        if (!maSachMoi.isEmpty()) {
            DanhSachSach dsSach = new DanhSachSach();
            dsSach.docFile("Java/DoAn/input/inputSach.txt");
            sachMoi = dsSach.timKiemTheoMa(maSachMoi);
            if (sachMoi == null) {
                System.out.println("Ma sach moi khong ton tai. Giu lai sach cu.");
                sachMoi = khoCanSua.getSach();
            }
        }
        ...
        Kho khoMoi = new Kho(maKho, sachMoi, soLuongMoi, mucTonToiThieuMoi, viTriMoi);
                ...
                if (kho != null) {
                    ...
                    System.out.printf("%-10s %-10s %-30s %-10s %-15s %-15s\n", "Ma Kho", "Ma Sach", "Ten Sach", "So Luong", "Ton Toi Thieu", "Vi Tri");
                     System.out.printf("%-10s %-10s %-30s %-10d %-15d %-15s\n",
                        kho.getMaKho(),
                        kho.getSach().getMaSach(),
                        kho.getSach().getTenSach(),
                        kho.getSoLuong(),
                 ...
                 if (khoTheoSach != null) {
                    ...
                    System.out.printf("%-10s %-10s %-30s %-10s %-15s %-15s\n", "Ma Kho", "Ma Sach", "Ten Sach", "So Luong", "Ton Toi Thieu", "Vi Tri");
                     System.out.printf("%-10s %-10s %-30s %-10d %-15d %-15s\n",
                        ...
                        khoTheoSach.getSach().getMaSach(),
                        khoTheoSach.getSach().getTenSach(),
